...
uniform vec3 u_coneROA[200];
uniform vec3 u_coneROB[200];
uniform vec3 u_coneROC[200];
uniform float u_rockCount;
uniform vec3 u_rocCol[200];
...
/*
	...
	*/
	...
	vec3 rconeNX;
		vec3 rcpa=pb+normalize(pb-pc)*0.5;
		//vec3 rcpb=rcpa-normalize(pb-pa);
		vec3 rcpb=pc;
		float rcra=0.05;
		float rcrb=0.05;
	...
	vec3 rconeNY;
		//vec3 rcpa=pb+normalize(pb-pa)*0.5;
		 rcpa=pc;
		 rcpb=normalize(-pb+u_vecCtrl[1])*1+rcpa;
		 rcra=0.05;
		 rcrb=0.05;
	...
	vec3 rconeNZ;
		//vec3 rcpa=pb+normalize(pb-pa)*0.5;
		 rcpa=pc;
		 rcpb=normalize(u_vecCtrl[2]-pb)*1+rcpa;
		 rcra=0.05;
		 rcrb=0.05;
